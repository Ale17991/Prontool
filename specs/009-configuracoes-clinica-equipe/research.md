...
- *Adicionar colunas em `public.tenants`*: rejeitado pelos motivos acima e pelo risco de quebrar policies existentes que pressupõem o shape atual.
- *JSON único `tenants.profile JSONB`*: rejeitado — perde validação por coluna (CNPJ, UF, formato de telefone), dificulta índices e contradiz o estilo do projeto (ver `tenant_integrations` que prefere colunas explícitas).
