...
- [ ] T022 [P] [US2] Contract test `tests/contract/integracoes.spec.ts` for `GET /api/configuracoes/integracoes` (list shape, role guard) and `GET|POST|DELETE /api/configuracoes/integracoes/[provider]` (404 on unknown provider, Zod validation per provider, redacted response, role guard admin-only) — per contracts/integracoes.md
- [ ] T023 [P] [US2] Contract test `tests/contract/integration-adapter.spec.ts` that iterates over `listProviders()` and for each registered adapter asserts: has `label`+`description`, `configSchema` validates a known-good fixture, `credentialsSchema` validates a fixture, `redactCredentials` never echoes input values (fuzz with 20 random "supersecret_*" strings), `handleDomainEvent` is a noop for unsupported event types — per contracts/integration-adapter.md
- [ ] T024 [P] [US2] Integration test `tests/integration/integrations/ghl/connect-disconnect.spec.ts` that POSTs a valid GHL config as admin, asserts row in `tenant_integrations`, asserts `audit_log` entry with `entity_id='<tenant>:ghl'`, reads the webhook/secret back through `GET` with redaction, then DELETEs with reason and asserts row removed + audit entry; also verifies recepcionista receives 403 on all three verbs

### Implementation for User Story 2

- [ ] T025 [P] [US2] Create GHL adapter `src/lib/integrations/ghl/adapter.ts` implementing `IntegrationAdapter<GhlConfig, GhlCredentials>` with `configSchema` (location_id, trigger_stage_name, 4 field maps), `credentialsSchema` ({ operations_pat, inbound_webhook_secret }), `redactCredentials` returning `{ operations_pat: '***', inbound_webhook_secret: '***' }`, stub `handleDomainEvent` that returns early (real implementation in T038)
- [ ] T026 [US2] Register GHL adapter in `src/lib/integrations/registry.ts` by importing `ghlAdapter` and adding to the `registry` object; update `ProviderId` union in `src/lib/integrations/types.ts` if needed
- [ ] T027 [P] [US2] Refactor `src/lib/integrations/ghl/create-contact.ts` to accept `(ctx: AdapterContext<GhlConfig, GhlCredentials>, input)` instead of reading `process.env.*` — build URL from `ctx.config` and Bearer from `ctx.credentials.operations_pat` per research.md R-006; keep the old env-based signature as a deprecated overload only if still referenced by `upsertPatientFromGhl`
- [ ] T028 [P] [US2] Create `src/lib/integrations/ghl/create-note.ts` exporting `createNoteInGhl(ctx, { contactId, body })` that POSTs to `{operations_url}/functions/v1/create-contact-note` with `AbortSignal.timeout(5000)`, per research.md R-007
- [ ] T029 [US2] Implement `src/app/api/configuracoes/integracoes/route.ts` (`GET`) — list status aggregate across `listProviders()` joined with `tenant_integrations` rows for the session's tenant; `requireRole(['admin'])`; response per contracts/integracoes.md
- [ ] T030 [US2] Implement `src/app/api/configuracoes/integracoes/[provider]/route.ts` (`GET`, `POST`, `DELETE`) — Zod validates `body.config`/`body.credentials` against `registry[provider].configSchema` / `credentialsSchema`; POST encrypts credentials (and `inbound_webhook_secret` separately into `webhook_secret_enc`), UPSERTs row, calls `recordIntegrationEvent({type:'integration.connect'|'reconfigure', ...})`; DELETE removes row and records `integration.disconnect`; all `requireRole(['admin'])`
- [ ] T031 [P] [US2] Create `src/app/(dashboard)/configuracoes/integracoes/page.tsx` — server component fetches `GET /api/configuracoes/integracoes` and renders a list/grid card per provider with connect/reconfigure/disconnect CTA buttons; `admin`-only (redirect others)
- [ ] T032 [P] [US2] Create `src/app/(dashboard)/configuracoes/integracoes/[provider]/page.tsx` — server component fetches detail `GET /api/configuracoes/integracoes/[provider]`, renders a `<ProviderForm>` client component that builds inputs from `config_schema` + `credentials_schema` (JSON Schema) dynamically, with reason textarea, Connect/Reconfigure/Disconnect buttons; shows redacted credentials in reconfigure mode so admin knows existing state is preserved
- [ ] T033 [US2] Update `src/app/(dashboard)/configuracoes/page.tsx` to add a "Integrações" tile linking to `/configuracoes/integracoes` (replaces the current ComingSoon placeholder mention of GHL)
- [ ] T034 [US2] Update `src/app/(dashboard)/_components/sidebar-integrations-badge.tsx` (created in T019) to actually render pills for connected integrations based on the prop populated in T020 — previously was stub returning `null` when empty
...
- [ ] T035 [P] [US3] Integration test `tests/integration/event-dispatch.spec.ts` that seeds a tenant with two enabled adapters (GHL + generic_webhook), dispatches a `patient.created` event, asserts both adapters are called in parallel via `Promise.allSettled`, and that one adapter's failure does not block the other
- [ ] T036 [P] [US3] Integration test `tests/integration/integrations/ghl/outbound-sync.spec.ts` covering: (a) patient create with GHL connected + mock 201 → `ghl_contact_id` persisted, no alert; (b) patient create with GHL connected + mock 500 → patient persisted, alert `integration_sync_failed` with `detail.provider='ghl'`, `detail.action='create_contact'`; (c) appointment create for patient with `ghl_contact_id` → note POST hits mock; (d) appointment create for patient without `ghl_contact_id` → note NOT attempted, no alert
- [ ] T037 [P] [US3] Integration test `tests/integration/integrations/ghl/outbound-respects-timeout.spec.ts` that mocks the proxy with a 10s delay, dispatches an event, and asserts the adapter's call aborted at 5s and the overall request returned within 8s (agreggated timeout from plan.md Performance Goals)

### Implementation for User Story 3

- [ ] T038 [US3] Implement `src/lib/core/events/dispatch.ts` fan-out logic: load `getEnabledIntegrations(tenantId)`, for each row call `decryptCredentials` and build `AdapterContext`, then `Promise.allSettled(adapters.map(a => withTimeout(a.handleDomainEvent(ctx, event), 5000)))`; on failure insert alert `integration_sync_failed` with `detail.provider` and `detail.action`; return `Array<{provider, ok, detail}>` — replaces the stub from T008
- [ ] T039 [US3] Implement `ghlAdapter.handleDomainEvent` in `src/lib/integrations/ghl/adapter.ts` with `switch(event.type)`: `'patient.created'` → call `createContactInGhl(ctx, event.patient)` and, on success, `supabase.from('patients').update({ghl_contact_id}).eq('id', event.patient.id)`; `'appointment.created'` → only if `event.patient.ghlContactId` is set, call `createNoteInGhl(ctx, { contactId, body: <formatted> })`; default → noop
- [ ] T040 [US3] Refactor `src/lib/core/patients/create-manual.ts` to remove the direct `createContactInGhl` call and instead call `publishDomainEvent(supabase, { type:'patient.created', patient: snapshot })` after INSERT succeeds; return `integrations_dispatched` array in the result for the API route to propagate
- [ ] T041 [US3] Update `src/app/api/pacientes/route.ts` `POST` to surface `integrations_dispatched` in the 201 body, and keep `ghl_synced` derived from `integrations_dispatched.find(i => i.provider==='ghl')?.ok ?? false` for back-compat per contracts/pacientes.md
- [ ] T042 [US3] Update `src/app/api/atendimentos/manual/route.ts` (from T016) to surface `integrations_dispatched` now that `publishDomainEvent` in `create-manual.ts` returns non-empty results; no behavior change for standalone tenants (still empty array)
- [ ] T043 [P] [US3] E2E test `tests/e2e/ghl-connected-sync.spec.ts` that wires the scenario from quickstart.md §Scenario 3 end-to-end (connect GHL via UI, create patient, verify mock received request, verify UI shows success pill)
