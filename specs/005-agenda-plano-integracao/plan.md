...
8. **Cookie de preferência de view** — chave `pronttu_atendimentos_view`. Server (`page.tsx`) lê via `cookies()` em SSR. Client (`atendimentos-toolbar.tsx`) escreve via `document.cookie` ao alternar. Ambos os lados sincronizados.
