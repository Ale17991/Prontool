...
- [ ] T006 **(em aberto — próximo passo da Foundation)** Domínios via **seed script** `scripts/seed-tiss-domains.ts` (+ `seed:tiss-domains`). Fonte oficial identificada: `Padrao_TISS_arquivos_auxiliares_202511.zip` (~178MB, ANS) contém as tabelas de domínio (junto ao histórico TUSS). Tarefa de dados dedicada: localizar o arquivo de domínios dentro do zip, parsear (xlsx/txt) e popular `tiss_domain_tables` (38/87/26/24/59/52/36/48/50/23/76/35) idempotente. Não bloqueia os contract tests; necessário para validação de conteúdo de US2 (dom. 87) e glosas de US5 (dom. 38).
