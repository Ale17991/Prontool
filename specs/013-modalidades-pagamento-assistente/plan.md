...
3. **Impacto em relatórios (US3, P3)** — view virtualizada `monthly_fixed_pay_lines(tenant_id, doctor_id, year, month, billing_date, amount_cents)` gera linhas de pagamento fixo *a partir* do dia de faturamento de cada mês, sem job/agendador. Relatórios `/relatorios/mensal`, `/relatorios/por-profissional/[doctorId]` e o resultado operacional consomem a view + agregam `appointment_assistants` (com filtro `removed_at IS NULL` + exclusão de estornados, FR-019/FR-022).
