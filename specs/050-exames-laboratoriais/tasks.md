...
- [ ] T034 Deploy: mergear em `master` + push (a integração GitHub da Supabase aplica a 0184 automaticamente no push — **não** aplicar à mão), rodar `pnpm seed:lab-ranges:prod`, e ligar o módulo `exames_lab` no `/admin` para os tenants alvo. Se der `MIDDLEWARE_INVOCATION_FAILED`, redeploy na Vercel **sem** cache de build.
