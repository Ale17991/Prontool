...
2. **Post-build scan** (`tools/scripts/scan-bundle-for-memed-keys.ts`) — executado após `next build`; faz grep recursivo nos arquivos sob `.next/static/` procurando padrões `mk_[A-Za-z0-9]{20,}` (formato típico de chave) e strings literais "MEMED\_". Falha CI se encontrar.
