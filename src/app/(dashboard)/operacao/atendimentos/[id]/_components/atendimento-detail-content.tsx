...
/**
 * Conteúdo do detalhe do atendimento (Server Component compartilhado).
 *
 * Renderizado tanto pela rota standalone (/operacao/atendimentos/[id])
 * quanto pelo Sheet lateral (interception @modal). Para que o conteúdo
 * fique idêntico em ambos os caminhos, o data-loading e o JSX vivem aqui;
 * cada wrapper só cuida do chrome (header de "Voltar" vs X do Sheet).
 *
 * - `fromModal=false` (default): mostra o botão "Voltar" + h1 padrão.
 * - `fromModal=true`: omite o "Voltar" porque o Sheet já tem o X de fechar.
 */
interface Props {
...
export async function AtendimentoDetailContent({ appointmentId, fromModal = false }: Props) {
  const session = await getSession()
  if (!session) notFound()

  const supabase = createSupabaseServerClient() as unknown as SupabaseClient<Database>
  const appointment = await loadAppointmentDetail(supabase, appointmentId)
          ...
          <h1
            className={
              fromModal
                ...
                : 'mt-3 text-2xl font-black tracking-tight text-slate-900'
            }
          >
            Atendimento
          </h1>
