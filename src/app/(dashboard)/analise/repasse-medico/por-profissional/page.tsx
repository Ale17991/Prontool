...
export default async function PorProfissionalPage({ searchParams }: PageProps) {
  ...
  // RBAC: admin/financeiro via can('report.read'); profissional_saude via
  // acesso ao próprio repasse — redireciona direto para o próprio detail.
  const isProfissionalSaude = session.role === 'profissional_saude'
  if (!isProfissionalSaude && !can(session.role, 'report.read')) {
    redirect('/operacao/atendimentos')
  }

  const period = resolvePeriod(searchParams)

  if (isProfissionalSaude) {
    ...
    const doctorRes = await sb
      ...
      .maybeSingle()
    const ownDoctorId = (doctorRes.data as { id: string } | null)?.id
    if (!ownDoctorId) redirect('/analise/repasse-medico')
    redirect(
      `/analise/repasse-medico/por-profissional/${ownDoctorId}?from=${period.from}&to=${period.to}`,
    )
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <RepasseSubNav active="por-profissional" />

      <Card>
        <CardContent className="space-y-3 p-4">
          <PeriodShortcuts
            basePath="/analise/repasse-medico/por-profissional"
  ...
  return (
    <Card className="flex flex-col">
      ...
      <CardContent className="flex flex-1 flex-col justify-between gap-4">
        ...
        <Link
          href={`/analise/repasse-medico/por-profissional/${row.doctorId}?from=${from}&to=${to}`}
