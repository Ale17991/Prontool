...
export default async function NovoPrecoPage() {
  ...
  if (!can(session.role, 'price.write')) redirect('/precos')
        ...
        <CardContent>
          {procedures.length === 0 ? (
            <p className="text-sm text-slate-500">
              ...
              <Link href="/procedimentos" className="font-semibold text-primary underline">
                procedimento
              </Link>{' '}
            ...
            <p className="text-sm text-slate-500">
              Cadastre ao menos um{' '}
              <Link href="/planos" className="font-semibold text-primary underline">
                convênio
              </Link>{' '}
              antes de criar um preço.
            </p>
