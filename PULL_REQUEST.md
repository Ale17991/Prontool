...
2. **Modo standalone + integrações multi-plataforma** (feature 002) — o Prontool opera 100% independente de qualquer CRM externo, e também conecta de forma opcional ao GoHighLevel, HubSpot (placeholder), RD Station (placeholder), Pipedrive (placeholder) ou a um webhook genérico — via um _registry_ de adapters pluggáveis.
...
- Webhook inbound genérico `/api/webhooks/[provider]` delega para `adapter.handleInboundWebhook(supabase, req)`. `/api/webhooks/ghl` permanece como _thin-forward_ para back-compat com URLs já configuradas.
...
1. **Drop `tenant_ghl_config`** — migrar `create-from-event.ts` e `extract-custom-fields.ts` para ler field*maps de `tenant_integrations.config` (requer estender `GhlConfig` com `field_map_patient*\*` e backfill). Migration 0041 já está como NOOP placeholder.
