...
import { buildSplit, type BillingPartner } from '@/lib/core/billing/partners'
...
describe('buildSplit', () => {
  it('monta o objeto do Asaas em reais, com o valor já arredondado aqui', () => {
    const out = buildSplit(partner(), 14990)
  ...
  it('parceiro sem carteira não divide — a cobrança sai inteira, não falha', () => {
    expect(buildSplit(partner({ asaasWalletId: null }), 14990)).toBeNull()
  })

  it('parceiro inativo não divide', () => {
    expect(buildSplit(partner({ status: 'inactive' }), 14990)).toBeNull()
  })

  it('sem parceiro não divide', () => {
    expect(buildSplit(null, 14990)).toBeNull()
  })
  ...
  it('split que arredonda para zero não vira split de zero reais', () => {
    ...
    expect(buildSplit(partner(), 1)).toBeNull()
  })
})
