...
## Recent Changes
- 011-cadastro-impostos: Added TypeScript 5.4 sobre Node.js 20 LTS (runtime Vercel). + Next.js 14.2 (App Router), `@supabase/ssr` 0.5, `@supabase/supabase-js` 2.45, Zod 3.23, Tailwind CSS 3.4, shadcn/ui (Radix primitives), `lucide-react`, Pino 9. **Sem novas deps**.
- 010-multi-tenant-ghl-calendar: Added TypeScript 5.4 sobre Node.js 20 LTS (runtime Vercel). + Next.js 14.2 (App Router), `@supabase/ssr` 0.5, `@supabase/supabase-js` 2.45, Zod 3.23, Tailwind 3.4, shadcn/ui (Radix), `date-fns` 4.1, `lucide-react`. **Sem novas deps de runtime**. Para o calendário usamos `date-fns` (já em deps) — para semana/mês/range; mini-calendário é componente próprio (não há libs no projeto que façam render de mês compacto, e adicionar uma só para isso é overkill).
