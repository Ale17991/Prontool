...
const PLANS = [
  { name: 'Unimed', tax: 300 },
  { name: 'Bradesco Saúde', tax: 300 },
  { name: 'SulAmérica', tax: 300 },
  { name: 'Amil', tax: 300 },
  { name: 'Hapvida', tax: 300 },
  { name: 'Particular', tax: 0 },
]
...
const ALLERGIES = [
  ['Dipirona', 'moderada'],
  ['Penicilina', 'grave'],
  ['Ácido acetilsalicílico', 'leve'],
  ['Látex', 'moderada'],
  ['Frutos do mar', 'grave'],
  ['Contraste iodado', 'moderada'],
]
const HISTORY = [
...
async function main() {
  ...
  log(
    `tenant=${tenantId}  actor=${ACTOR}  encKey=${HAS_KEY ? 'sim' : 'NÃO (pacientes serão pulados)'}`,
  )
  ...
  await section('planos', async () => {
    for (const p of PLANS) {
      const ex = await sb
        ...
        .maybeSingle()
      if (ex.data) {
        planIds[p.name] = ex.data.id
        continue
      }
      const ins = await sb
        .from('health_plans')
        .insert({ tenant_id: tenantId, name: p.name, tax_rate_bps: p.tax })
        .select('id')
        .single()
    ...
    for (const p of PROCS) {
      ...
      const ins = await sb
        ...
        .insert({
          ...
          tuss_code: tuss,
          display_name: p.name,
          covered_by_plan: p.covered,
          default_amount_cents: p.amount,
          active: true,
        })
        .select('id')
        .single()
  ...
  await section('precos', async () => {
    ...
    for (const [pi, planName] of priceMatrix) {
      const procedure_id = procIds[pi]
      const plan_id = planIds[planName]
      ...
      if (r.error) {
        console.error(`   ⚠ preco: ${r.error.message}`)
      } else n++
    }
    log(`  precos novos: ${n}`)
  })
  ...
  await section('pacientes', async () => {
    if (!HAS_KEY) {
      log('  pacientes: PULADO (sem chave)')
      return
    }
    ...
    for (let i = existing; i < PATIENTS.length; i++) {
      ...
      await createPatientManually(sb, {
        ...
        cpf: p.cpf,
        phone: p.phone,
        email: `${p.name.split(' ')[0].toLowerCase()}@exemplo.com`,
        birthDate: p.birth,
        sex: p.sex as any,
        planId: planIds[PLANS[i % PLANS.length].name] ?? null,
      })
    }
    const all = await sb
      .from('patients')
  ...
  const apptPlan: Array<{ proc: number; plan: string; day: number }> = [
    { proc: 0, plan: 'Unimed', day: -40 },
    { proc: 0, plan: 'Bradesco Saúde', day: -33 },
    { proc: 2, plan: 'Unimed', day: -26 },
    { proc: 0, plan: 'Particular', day: -19 },
    { proc: 3, plan: 'Particular', day: -12 },
    { proc: 0, plan: 'SulAmérica', day: -6 },
    { proc: 1, plan: 'Unimed', day: -2 },
    { proc: 0, plan: 'Amil', day: 1 },
    { proc: 0, plan: 'Hapvida', day: 2 },
    { proc: 4, plan: 'Particular', day: 4 },
    { proc: 5, plan: 'Particular', day: 7 },
    { proc: 0, plan: 'Particular', day: 10 },
  ]
  await section('atendimentos', async () => {
    if (!havePatients || !haveDoctors) {
      log('  atendimentos: PULADO (faltam pacientes/médicos)')
      return
    }
    const existing = await countOf('appointments', tenantId)
    if (existing >= apptPlan.length) {
      log(`  atendimentos já: ${existing}`)
      return
    }
    ...
    for (let i = existing; i < apptPlan.length; i++) {
      ...
      try {
        const res = await createAppointmentManually(sb, {
          ...
          doctorId: doctorIds[di],
          procedures: [{ procedureId: procIds[a.proc], planId: planIds[a.plan] }],
          appointmentAt: iso(daysFromNow(a.day, 8 + slot[di], 0)),
          durationMinutes: 30,
          observacoes: 'Atendimento de demonstração',
          addToTreatmentPlan: true,
        })
        created++
        if (a.day < 0) {
          try {
            await markAppointmentRealized(sb, {
        ...
        console.error(`   ⚠ appt ${i}: ${(e as Error).message}`)
      }
    }
  ...
  await section('pagamentos', async () => {
    if (!havePatients) {
      log('  pagamentos: PULADO')
      return
    }
    if ((await countOf('payment_records', tenantId)) >= 6) {
      log('  pagamentos já ok')
      return
    }
    ...
    for (let i = 0; i < 6; i++) {
      try {
        await createPaymentRecord(sb, {
          ...
          actorUserId: ACTOR,
          patientId: patientIds[i % patientIds.length],
          totalAmountCents: [30000, 40000, 18000, 25000, 12000, 20000][i],
          paymentMethod: methods[i] as any,
          installmentsCount: i % 2 === 0 ? 1 : 3,
          initialStatus: i % 2 === 0 ? 'pago' : 'pendente',
          notes: i % 2 === 0 ? 'Pago no atendimento' : 'Parcelado',
        })
      } catch (e) {
        console.error(`   ⚠ pagamento ${i}: ${(e as Error).message}`)
      }
    }
    log('  pagamentos: ok')
  })
  ...
  await section('prontuario', async () => {
    ...
    if ((await countOf('clinical_records', tenantId)) < 6)
      ok(
        'clinical_records',
        await sb.from('clinical_records').insert(
          Array.from({ length: 6 }, (_, i) => ({
            tenant_id: tenantId,
            patient_id: pid(i),
            created_by: ACTOR,
            type: 'texto',
            title: `Evolução clínica ${i + 1}`,
            content:
    ...
    if ((await countOf('patient_allergies', tenantId)) < 6)
      ok(
        'allergies',
        await sb.from('patient_allergies').insert(
          ALLERGIES.map(([substance, severity], i) => ({
            tenant_id: tenantId,
            patient_id: pid(i),
            reported_by: ACTOR,
            substance,
    ...
    if ((await countOf('patient_diagnoses', tenantId)) < 6)
      ok(
        'diagnoses',
        await sb.from('patient_diagnoses').insert(
          CIDS.slice(0, 6).map(([cid10_code, cid10_description], i) => ({
            tenant_id: tenantId,
            patient_id: pid(i),
            diagnosed_by: ACTOR,
            cid10_code,
    ...
    if ((await countOf('patient_history', tenantId)) < 6)
      ok(
        'history',
        await sb.from('patient_history').insert(
          HISTORY.map(([category, description], i) => ({
            tenant_id: tenantId,
            patient_id: pid(i),
            reported_by: ACTOR,
            category,
  ...
  await section('despesas', async () => {
    if ((await countOf('expenses', tenantId)) >= 6) {
      log('  despesas já ok')
      return
    }
    ok(
      ...
      await sb.from('expenses').insert(
        EXPENSES.map((e, i) => ({
          ...
          competence_date: daysFromNow(-15 + i)
            .toISOString()
            .slice(0, 10),
  ...
  await section('tarefas', async () => {
    if ((await countOf('tasks', tenantId)) >= 6) {
      log('  tarefas já ok')
      return
    }
    const titles: Array<[string, string]> = [
      ['Confirmar agenda da semana', 'alta'],
      ['Ligar para paciente sobre exame', 'normal'],
      ['Renovar convênio Unimed', 'normal'],
      ['Comprar material de consumo', 'baixa'],
      ['Revisar repasses do mês', 'urgente'],
      ['Atualizar tabela de preços', 'baixa'],
    ]
    ok(
      'tasks',
      await sb.from('tasks').insert(
        titles.map(([title, priority], i) => ({
          tenant_id: tenantId,
          assigned_to: ACTOR,
          assigned_by: ACTOR,
          created_by: ACTOR,
          ...
          completed_at: i % 3 === 0 ? daysFromNow(-1).toISOString() : null,
          completed_by: i % 3 === 0 ? ACTOR : null,
          due_date: daysFromNow(2 + i)
            .toISOString()
            .slice(0, 10),
        })),
      ),
  ...
  await section('lembretes', async () => {
    if ((await countOf('appointment_reminders', tenantId)) >= 5) {
      log('  lembretes já ok')
      return
    }
    const fut = await sb
  ...
  await section('portal', async () => {
    const taken = await sb
      ...
      .select('tenant_id')
      .eq('public_booking_slug', PORTAL_SLUG)
      .neq('tenant_id', tenantId)
      .maybeSingle()
    const slug = taken.data ? `${PORTAL_SLUG}-${tenantId.slice(0, 4)}` : PORTAL_SLUG
    ok(
      'profile',
      await sb.from('tenant_clinic_profile').upsert(
        {
          tenant_id: tenantId,
    ...
    if (havePatients && (await countOf('patient_measurements', tenantId)) < 15) {
      const metrics: Array<[string, string, number, number]> = [
        ['glicemia_jejum', 'mg/dL', 140, -3],
        ['hba1c', '%', 8.2, -0.15],
        ['colesterol_total', 'mg/dL', 220, -4],
      ]
      const rows: any[] = []
        ...
        for (const [metric_type, unit, start, step] of metrics)
          for (let k = 0; k < 6; k++)
            rows.push({
              ...
              metric_type,
              unit,
              value: Number((start + step * k).toFixed(2)),
              measured_at: daysFromNow(-150 + k * 25)
                .toISOString()
                .slice(0, 10),
            })
  ...
  await section('agendamento-publico', async () => {
    if (!haveDoctors) {
      log('  agendamento público: PULADO')
      return
    }
    if ((await countOf('public_booking_doctors', tenantId)) >= 5) {
      log('  agendamento público já ok')
      return
    }
    ok(
      'public_booking',
      await sb.from('public_booking_doctors').insert(
        doctorIds.slice(0, 6).map((doctor_id, i) => ({
          tenant_id: tenantId,
          doctor_id,
          display_order: i,
          bio: `${DOCTORS[i].spec} — atendimento humanizado.`,
  ...
  await section('prescricoes', async () => {
    if (!havePatients || !haveDoctors) {
      log('  prescrições: PULADO')
      return
    }
    if ((await countOf('prescription_records', tenantId)) >= 5) {
      log('  prescrições já ok')
      return
    }
    const r = await sb.from('prescription_records').insert(
      Array.from({ length: 5 }, (_, i) => ({
        tenant_id: tenantId,
        patient_id: patientIds[i % patientIds.length],
        doctor_id: doctorIds[i % doctorIds.length],
        created_by_user_id: ACTOR,
        memed_prescription_id: `demo-rx-${1000 + i}`,
        status: 'issued',
  ...
  await section('notificacoes', async () => {
    const r = await sb.rpc('generate_user_notifications', {
      p_tenant_id: tenantId,
      p_user_id: ACTOR,
    })
    log(
      r.error
        ? `  notificações: pulado (${r.error.message.slice(0, 60)})`
  ...
  log('\n✅ seed concluído.')
  log(
    `   portal demo: paciente ex. CPF ${PATIENTS[0].cpf} / nasc. ${PATIENTS[0].birth.split('-').reverse().join('/')}`,
  )
}
