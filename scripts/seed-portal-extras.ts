...
const monthsAgoIso = (n: number) => { const d = new Date(); d.setMonth(d.getMonth() - n); d.setHours(10, 0, 0, 0); return d.toISOString() }
...
async function main() {
  ...
  const { data: au } = await sb.from('user_tenants').select('user_id').eq('tenant_id', tenant.id).eq('role', 'admin').limit(1).maybeSingle()
  ...
  const patient = list.items.find((p) => p.fullName.toLowerCase().includes(PATIENT_NAME.toLowerCase()))
  if (!patient) throw new Error(`Paciente "${PATIENT_NAME}" não encontrado na clínica.`)
  const patientId = patient.id
  console.log(`[extras] clínica ${CLINIC} (${tenant.id}) · paciente ${patient.fullName} (${patientId})`)

  // portal + seções
  await updatePatientPortalConfig(sb, tenant.id, { patientPortalEnabled: true, publicBookingSlug: tenant.slug })
  for (const section_key of ['orientacoes', 'treino', 'dieta']) {
    await sb.from('tenant_portal_sections' as never).upsert({ tenant_id: tenant.id, section_key, enabled: true } as never, { onConflict: 'tenant_id,section_key' })
  }

  // medições (6 meses)
  for (let i = 5; i >= 0; i--) {
    const idx = 5 - i
    await createVitalSigns(sb, { tenantId: tenant.id, patientId, measuredAt: monthsAgoIso(i), weightGrams: Math.round((WEIGHT_START - idx * 1.3) * 1000), heightCm: HEIGHT_CM, actorUserId })
    await recordMeasurement(sb, { tenantId: tenant.id, patientId, metricType: 'glicemia_jejum', value: GLIC[idx]!, measuredAt: monthsAgo(i), actorUserId })
    await recordMeasurement(sb, { tenantId: tenant.id, patientId, metricType: 'hba1c', value: HBA1C[idx]!, measuredAt: monthsAgo(i), actorUserId })
    await recordMeasurement(sb, { tenantId: tenant.id, patientId, metricType: 'colesterol_total', value: COLEST[idx]!, measuredAt: monthsAgo(i), actorUserId })
  }

  // metas
  const currentWeight = WEIGHT_START - 5 * 1.3
  await setGoal(sb, { tenantId: tenant.id, patientId, metricType: 'peso_kg', direction: 'decrease', targetValue: Math.round((currentWeight - 4) * 10) / 10, actorUserId })
  await setGoal(sb, { tenantId: tenant.id, patientId, metricType: 'glicemia_jejum', direction: 'decrease', targetValue: 90, actorUserId })

  // treino + dieta + orientação
  await createWorkoutPlan(sb, { tenantId: tenant.id, patientId, title: 'Treino de hipertrofia — A/B/C', notes: 'Treinar 3x/semana, 1 dia de descanso entre treinos.', sessions: WORKOUT, actorUserId })
  await createDietPlan(sb, { tenantId: tenant.id, patientId, title: 'Plano alimentar — manutenção', notes: 'Beber 2L de água/dia. Evitar açúcar.', meals: DIET, actorUserId })
  await createCareNote(sb, { tenantId: tenant.id, patientId, body: 'Ótima evolução! Manter o treino e a dieta combinados e retornar em 60 dias com novos exames.', actorUserId })

  const [yy, mm, dd] = (patient.birthDate ?? '').split('-')
  console.log('[extras] concluído.')
  console.log(`  Portal: /paciente/${tenant.slug ?? '<slug>'}`)
  console.log(`  Login: CPF ${patient.cpf || '(sem CPF)'} · Senha (nascimento) ${dd && mm && yy ? `${dd}${mm}${yy}` : '(sem data de nascimento — preencha no cadastro)'}`)
}

main().catch((err) => { console.error('[extras] FALHOU:', err); process.exit(1) })
